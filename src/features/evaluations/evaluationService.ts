...
export async function fetchEvaluations(): Promise<{ stats: EvaluationStats; rows: EvaluationRow[] }> {
  const { data, error } = await supabase
    .from('evaluations')
    .select(`
      id, rating, feedback, would_recommend, submitted_at,
      registrations ( name, activity_id, activities ( title ) )
    `)
  ...
  const rows: EvaluationRow[] = (data || []).map((e: any) => ({
    ...
    participantName: e.registrations?.name || '—',
    activityTitle: e.registrations?.activities?.title || '—',
  }))
